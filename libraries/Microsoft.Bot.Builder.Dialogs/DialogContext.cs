...
namespace Microsoft.Bot.Builder.Dialogs
{
    public class DialogContext
    {
        ...
        internal DialogContext(List<Dialog> dialogs, ITurnContext turnContext, DialogState state)
        {
        ...
        public Dialog Find(string dialogId)
        {
            ...
            return Dialogs.Where(dialog => dialog.Id == dialogId).FirstOrDefault();
        }
        ...
        public async Task<DialogTurnResult> BeginDialogAsync(string dialogId, object options = null, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            // Lookup dialog
            var dialog = Dialogs.Where(dlg => dlg.Id == dialogId).FirstOrDefault();
            if (dialog == null)
            {
        ...
        public async Task<DialogTurnResult> ContinueDialogAsync(CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            {
                // Lookup dialog
                var dialog = Dialogs.Find(dlg => dlg.Id == ActiveDialog.Id);
                if (dialog == null)
                {
        ...
        private async Task EndActiveDialogAsync(DialogReason reason, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            {
                // Lookup dialog
                var dialog = Dialogs.Find(dlg => dlg.Id == instance.Id);
                if (dialog != null)
                {
